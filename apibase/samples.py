...
class ApiritifSampleExtractor(object):
    ...
    def parse_recording(self, recording, test_case_sample):
        ...
        for item in recording:
            if isinstance(item, apibase.Request):
                self._parse_request(item)
            elif isinstance(item, apibase.TransactionStarted):
                self._parse_transaction_started(item)
            elif isinstance(item, apibase.TransactionEnded):
                self._parse_transaction_ended(item)
            elif isinstance(item, apibase.Assertion):
                self._parse_assertion(item)
            elif isinstance(item, apibase.AssertionFailure):
                self._parse_assertion_failure(item)
            else:
                raise ValueError("Unknown kind of event in apibase recording: %s" % item)

        if len(self.active_transactions) != 1:
            # TODO: shouldn't we auto-balance them?
            raise ValueError("Can't parse apibase recordings: unbalanced transactions")
