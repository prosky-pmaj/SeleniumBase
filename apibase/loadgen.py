...
import apibase
from apibase.samples import ApiritifSampleExtractor, Sample, PathComponent
...
class Worker(ThreadPool):
    ...
    def run_nose(self, params):
        ...
        # argv.extend(['--with-apibase', '--nocapture', '--exe', '--nologcapture'])
...
class ApiritifPlugin(Plugin):
    ...
    def _process_apiritif_samples(self, sample):
        ...
        recording = apibase.recorder.pop_events(from_ts=self.start_time, to_ts=self.end_time)
        if not recording:
...
def setup_logging(params):
    ...
    apibase.http.log.setLevel(logging.WARNING)
    if params.verbose:
    ...
    apibase.log.setLevel(logging.INFO)  # TODO: do we need to include apibase debug logs in verbose mode?
